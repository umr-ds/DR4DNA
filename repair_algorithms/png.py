    ...
    * <http://littlesvr.ca/apng/>
    ...
    def _read(self):
        ...
        if not self.ihdr_len == 13:
            raise kaitaistruct.ValidationNotEqualError(13, self.ihdr_len, self._io, "/seq/1")
        self.ihdr_type = self._io.read_bytes(4)
        if not self.ihdr_type == b"\x49\x48\x44\x52":
            raise kaitaistruct.ValidationNotEqualError(
                b"\x49\x48\x44\x52", self.ihdr_type, self._io, "/seq/2"
            )
    ...
    class CompressedTextChunk(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.compression_method = KaitaiStream.resolve_enum(
                Png.CompressionMethods, self._io.read_u1()
            )
        ...
        def _read(self):
            ...
            if not self.width >= 1:
                raise kaitaistruct.ValidationLessThanError(
                    1, self.width, self._io, "/types/frame_control_chunk/seq/1"
                )
            if not self.width <= self._root.ihdr.width:
                raise kaitaistruct.ValidationGreaterThanError(
                    self._root.ihdr.width, self.width, self._io, "/types/frame_control_chunk/seq/1"
                )
            self.height = self._io.read_u4be()
            if not self.height >= 1:
                raise kaitaistruct.ValidationLessThanError(
                    1, self.height, self._io, "/types/frame_control_chunk/seq/2"
                )
            if not self.height <= self._root.ihdr.height:
                raise kaitaistruct.ValidationGreaterThanError(
                    self._root.ihdr.height,
                    self.height,
                    self._io,
                    "/types/frame_control_chunk/seq/2",
                )
            self.x_offset = self._io.read_u4be()
            if not self.x_offset <= (self._root.ihdr.width - self.width):
                raise kaitaistruct.ValidationGreaterThanError(
                    (self._root.ihdr.width - self.width),
                    self.x_offset,
                    self._io,
                    "/types/frame_control_chunk/seq/3",
                )
            self.y_offset = self._io.read_u4be()
            if not self.y_offset <= (self._root.ihdr.height - self.height):
                raise kaitaistruct.ValidationGreaterThanError(
                    (self._root.ihdr.height - self.height),
                    self.y_offset,
                    self._io,
                    "/types/frame_control_chunk/seq/4",
                )
    ...
    class InternationalTextChunk(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.translated_keyword = (self._io.read_bytes_term(0, False, True, True)).decode(
                "UTF-8"
            )
...
if __name__ == "__main__":
    x = Png.from_file("../output_files.png")
    print(x)
