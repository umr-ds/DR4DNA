...
class Zip(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None, start_offset=0):
        ...
        self.start = (
            self._io.pos() + start_offset
        )  # + (_parent.start if _parent is not None else 0)
    ...
    def _read(self, start_offset):
        ...
        while not self._io.is_eof():
            self.sections.append(
                Zip.PkSection(self._io, self, self._root, start_offset=start_offset)
            )
            i += 1
        ...
        def _read(self, start_offset=0):
            ...
            def read_except(len_body):
                try:
                    ...
                    allowed_bytes = int(
                        ex.args[0].split("but only ")[1].split(" bytes available")[0]
                    )
                    raise InvalidDataException(
                        "/types/compressed_data/invalid",
                        len_body_compressed_io_pos,
                        expected=allowed_bytes,
                    )
    ...
    class ExtraField(KaitaiStruct):
        ...
        def _read(self, start_offset=0):
            def read_except(len_body):
                try:
                    ...
                    raise InvalidDataException(
                        "/types/raw_body/invalid", len_body_io_pos, expected=allowed_bytes
                    )
            ...
            if _on == Zip.ExtraCodes.ntfs:
                ...
                self.body = Zip.ExtraField.Ntfs(
                    _io__raw_body, self, self._root, start_offset=start_offset + len_body_io_pos
                )
            elif _on == Zip.ExtraCodes.extended_timestamp:
                self._raw_body = read_except(self.len_body)
                _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                self.body = Zip.ExtraField.ExtendedTimestamp(
                    _io__raw_body, self, self._root, start_offset=start_offset + len_body_io_pos
                )
            elif _on == Zip.ExtraCodes.infozip_unix_var_size:
                self._raw_body = read_except(self.len_body)
                _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                self.body = Zip.ExtraField.InfozipUnixVarSize(
                    _io__raw_body, self, self._root, start_offset=start_offset + len_body_io_pos
                )
            else:
        ...
        class Ntfs(KaitaiStruct):
            ...
            def __init__(self, _io, _parent=None, _root=None, start_offset=0):
                ...
                self._parent = _parent
                self.start = (
                    self._io.pos() + (_parent.start if _parent is not None else 0) + start_offset
                )
                self._root = _root if _root else self
                self._read(start_offset=start_offset)
                self.end = (
            ...
            def _read(self, start_offset):
                ...
                while not self._io.is_eof():
                    self.attributes.append(
                        Zip.ExtraField.Ntfs.Attribute(
                            self._io, self, self._root, start_offset=start_offset
                        )
                    )
                    i += 1
            ...
            class Attribute(KaitaiStruct):
                ...
                def _read(self, start_offset):
                    ...
                    if _on == 1:
                        ...
                        _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                        self.body = Zip.ExtraField.Ntfs.Attribute1(
                            _io__raw_body, self, self._root, start_offset
                        )
                    else:
        ...
        class ExtendedTimestamp(KaitaiStruct):
            ...
            def _read(self, start_offset):
                ...
                self.flags = Zip.ExtraField.ExtendedTimestamp.InfoFlags(
                    _io__raw_flags, self, self._root, start_offset
                )
    ...
    def find_signtures(input_bytes: bytes):
        try:
            res = input_bytes.find(
                re.search(
                    rb"\x50\x4b\x03\x04|\x50\x4b\x01\x02|\x50\x4b\x05\x06|\x50\x4b\x07\x08",
                    input_bytes,
                ).group(0)
            )
        except:
    ...
    class CentralDirEntry(KaitaiStruct):
        ...
        def _read(self, start_offset):
            ...
            self.compression_method = KaitaiStream.resolve_enum(
                Zip.Compression, self._io.read_u2le()
            )
            ...
            try:
                ...
                raise InvalidDataException(
                    "/types/filename/invalid", len_filename_io_pos, expected=allowed_bytes
                )
            self.file_name = self._raw_file_name.decode("UTF-8", "ignore")
            ...
            if len(self._raw_file_name) != self.len_file_name or position_of_signature is not None:
                ...
                expected_val = self.len_file_name - max(
                    0,
                    self.len_file_name - position_of_signature - self.len_extra - self.len_comment,
                )
                raise InvalidDataException(
                    "/types/filename/invalid", len_filename_io_pos, expected=expected_val
                )
            ...
            if position_of_signature is not None:
                expected_val = self.len_extra - max(
                    0, self.len_extra - position_of_signature - self.len_comment
                )
                raise InvalidDataException(
                    "/types/extra/invalid", len_extra_io_pos, expected=expected_val
                )
            ...
            try:
                ...
                allowed_bytes = int(ex.args[0].split("but only ")[1].split(" bytes available")[0])
                raise InvalidDataException(
                    "/types/comment/invalid", len_comment_io_pos, expected=allowed_bytes
                )
            ...
            if diff_position is not None and position_of_signature is None:
                ...
                raise InvalidDataException(
                    "/types/comment/invalid", len_comment_io_pos, expected=expected_val
                )
                ...
                expected_val = self.len_comment - max(
                    0, self.len_comment - min(diff_position, position_of_signature)
                )
                raise InvalidDataException(
                    "/types/comment/invalid", len_comment_io_pos, expected=expected_val
                )
        ...
        def _read(self, start_offset=0):
            ...
            if self.magic != b"\x50\x4B":
                raise kaitaistruct.ValidationNotEqualError(
                    b"\x50\x4B", self.magic, self._io, "/types/pk_section/seq/0"
                )
            self.section_type = self._io.read_u2le()
            _on = self.section_type
            if _on == 513:
                self.body = Zip.CentralDirEntry(
                    self._io, self, self._root, start_offset=start_offset
                )
            elif _on == 1027:
                self.body = Zip.LocalFile(self._io, self, self._root, start_offset=start_offset)
            elif _on == 1541:
                self.body = Zip.EndOfCentralDir(
                    self._io, self, self._root, start_offset=start_offset
                )
            elif _on == 2055:
                self.body = Zip.DataDescriptor(
                    self._io, self, self._root, start_offset=start_offset
                )
            else:
                raise kaitaistruct.ValidationNotAnyOfError(
                    self.section_type, self._io, "/types/pk_section/seq/1"
                )
    ...
    class LocalFileHeader(KaitaiStruct):
        ...
        def _read(self, start_offset=0):
            ...
            if len(self._raw_file_name) != len(self.file_name):
                diff_position = Zip.find_first_difference(
                    self._raw_file_name, self.file_name.encode("UTF-8")
                )
            if (
                diff_position is not None
                or allowed_bytes is not None
                or position_of_signature is not None
            ):
                min_error_pos = min(
                    [
                        x
                        for x in [diff_position, allowed_bytes, position_of_signature]
                        if x is not None
                    ]
                )
                expected_val = self.len_file_name - (
                    self.len_file_name - min_error_pos - self.len_extra
                )
                raise InvalidDataException(
                    "/types/filename/invalid", len_filename_io_pos, expected=expected_val
                )
            ...
            try:
                ...
                allowed_bytes = int(ex.args[0].split("but only ")[1].split(" bytes available")[0])
                raise InvalidDataException(
                    "/types/raw_extra/invalid", len_extra_io_pos, expected=allowed_bytes
                )
            position_of_signature = Zip.find_signtures(self._raw_extra)
            if allowed_bytes is not None or position_of_signature is not None:
                min_error_pos = min(
                    [x for x in [allowed_bytes, position_of_signature] if x is not None]
                )
                expected_val = self.len_file_name - (
                    self.len_file_name - min_error_pos - self.len_extra
                )
                raise InvalidDataException(
                    "/types/raw_extra/invalid", len_filename_io_pos, expected=expected_val
                )
            _io__raw_extra = KaitaiStream(BytesIO(self._raw_extra))
            self.extra = Zip.Extras(
                _io__raw_extra, self, self._root, start_offset=start_offset + raw_extra_start_pos
            )
        ...
        class GpFlags(KaitaiStruct):
            ...
            def deflated_mode(self):
                ...
                if (self._parent.compression_method == Zip.Compression.deflated) or (
                    ...
                    self._m_deflated_mode = KaitaiStream.resolve_enum(
                        Zip.LocalFileHeader.GpFlags.DeflateMode, self.comp_options_raw
                    )
            ...
            def imploded_dict_byte_size(self):
                ...
                if self._parent.compression_method == Zip.Compression.imploded:
                    self._m_imploded_dict_byte_size = (
                        8 if (self.comp_options_raw & 1) != 0 else 4
                    ) * 1024
    ...
    class EndOfCentralDir(KaitaiStruct):
        ...
        def _read(self):
            ...
            try:
                ...
                allowed_bytes = int(ex.args[0].split("but only ")[1].split(" bytes available")[0])
                raise InvalidDataException(
                    "/types/comment_len/invalid", len_comment_io_pos, expected=allowed_bytes
                )
            position_of_signature = Zip.find_signtures(comment)
