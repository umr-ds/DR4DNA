...
class Bmp(KaitaiStruct):
    ...
    class BitmapV5Extension(KaitaiStruct):
        ...
        def has_profile(self):
            ...
            self._m_has_profile = (
                self._parent.bitmap_v4_ext.color_space_type == Bmp.ColorSpace.profile_linked
            ) or (self._parent.bitmap_v4_ext.color_space_type == Bmp.ColorSpace.profile_embedded)
    ...
    class BitmapInfoExtension(KaitaiStruct):
        ...
        def _read(self):
            ...
            if self._parent.extends_os2_2x_bitmap:
                self.os2_compression = KaitaiStream.resolve_enum(
                    Bmp.Os2Compressions, self._io.read_u4le()
                )
    ...
    class BitmapHeader(KaitaiStruct):
        ...
        def __init__(self, len_header, _io, _parent=None, _root=None):
            ...
            self.start = self._io.pos() + 4 + (_parent.start if _parent is not None else 0)
            self._read()
            self.end = self._io.pos() + 4 + (_parent.start if _parent is not None else 0)
        ...
        def _read(self):
            ...
            if self.is_color_mask_here:
                self.color_mask = Bmp.ColorMask(
                    self.len_header != Bmp.HeaderType.bitmap_v2_info_header.value,
                    self._io,
                    self,
                    self._root,
                )
        ...
        def extends_os2_2x_bitmap(self):
            ...
            self._m_extends_os2_2x_bitmap = (
                self.len_header == Bmp.HeaderType.os2_2x_bitmap_header.value
            )
            ...
            self._m_uses_fixed_palette = (
                not (
                    (
                        ...
                        or (self.bits_per_pixel == 24)
                        or (self.bits_per_pixel == 32)
                    )
                )
            ) and (
                not (
                    (
                        ...
                        and (
                            (
                                (self.bitmap_info_ext.compression == Bmp.Compressions.jpeg)
                                or (self.bitmap_info_ext.compression == Bmp.Compressions.png)
                            )
                        )
                    )
                )
        ...
        def image_height(self):
            ...
            self._m_image_height = (
                -(self.image_height_raw) if self.image_height_raw < 0 else self.image_height_raw
            )
        ...
        def is_color_mask_here(self):
            ...
            self._m_is_color_mask_here = (
                (self.len_header == Bmp.HeaderType.bitmap_v2_info_header.value)
                or (self.len_header == Bmp.HeaderType.bitmap_v3_info_header.value)
                or (self.extends_bitmap_v4)
            )
        ...
        def _read(self):
            ...
            for _i in range(
                (
                    ...
                    else self.num_colors_present
                )
            ):
                self.colors.append(
                    Bmp.RgbRecord(self.has_reserved_field, self._io, self, self._root)
                )
        ...
        def _read(self):
            ...
            if not self.file_type == b"\x42\x4D":
                raise kaitaistruct.ValidationNotEqualError(
                    b"\x42\x4D", self.file_type, self._io, "/types/file_header/seq/0"
                )
    ...
    class BitmapInfo(KaitaiStruct):
        ...
        def __init__(self, _io, _parent=None, _root=None):
            ...
            self._parent = _parent
            self._root = _root if _root else self
            self.start = 14 + self._io.pos()
            self._read()
            self.end = 14 + self._io.pos()
        ...
        def _read(self):
            ...
            if not (self._io.is_eof()):
                ...
                self.color_table = Bmp.ColorTable(
                    not (self.header.is_core_header),
                    (
                        self.header.bitmap_info_ext.num_colors_used
                        if self.header.extends_bitmap_info
                        else 0
                    ),
                    _io__raw_color_table,
                    self,
                    self._root,
                )
        ...
        def is_color_mask_given(self):
            ...
            self._m_is_color_mask_given = (
                ...
                and (
                    (
                        (self.header.bitmap_info_ext.compression == Bmp.Compressions.bitfields)
                        or (
                            self.header.bitmap_info_ext.compression
                            == Bmp.Compressions.alpha_bitfields
                        )
                    )
                )
        ...
        def color_mask_given(self):
            ...
            if self.is_color_mask_given:
                self._m_color_mask_given = (
                    self.color_mask if self.is_color_mask_here else self.header.color_mask
                )
            ...
            self._m_color_mask_blue = (
                ...
                if self.is_color_mask_given
                    ...
                    if self.header.bits_per_pixel == 16
                        ...
                        255
                        if (
                            (self.header.bits_per_pixel == 24) or (self.header.bits_per_pixel == 32)
                        )
                        else 0
                    )
                )
        ...
        def color_mask_alpha(self):
            ...
            self._m_color_mask_alpha = (
                self.color_mask_given.alpha_mask
                if ((self.is_color_mask_given) and (self.color_mask_given.has_alpha_mask))
                else 0
            )
...
if __name__ == "__main__":
    x = Bmp.from_file("/home/michael/Bilder/a.bmp")
    print(x)
