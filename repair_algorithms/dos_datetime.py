...
class DosDatetime(KaitaiStruct):
    ...
    class Time(KaitaiStruct):
        ...
        def _read(self):
            ...
            if not self.second_div_2 <= 29:
                raise kaitaistruct.ValidationGreaterThanError(
                    29, self.second_div_2, self._io, "/types/time/seq/0"
                )
            self.minute = self._io.read_bits_int_le(6)
            if not self.minute <= 59:
                raise kaitaistruct.ValidationGreaterThanError(
                    59, self.minute, self._io, "/types/time/seq/1"
                )
            self.hour = self._io.read_bits_int_le(5)
            if not self.hour <= 23:
                raise kaitaistruct.ValidationGreaterThanError(
                    23, self.hour, self._io, "/types/time/seq/2"
                )
        ...
        def padded_second(self):
            ...
            self._m_padded_second = ("0" if self.second <= 9 else "") + str(self.second)
        ...
        def padded_minute(self):
            ...
            self._m_padded_minute = ("0" if self.minute <= 9 else "") + str(self.minute)
        ...
        def padded_hour(self):
            ...
            self._m_padded_hour = ("0" if self.hour <= 9 else "") + str(self.hour)
    ...
    class Date(KaitaiStruct):
        ...
        def _read(self):
            ...
            if not self.day >= 1:
                raise kaitaistruct.ValidationLessThanError(
                    1, self.day, self._io, "/types/date/seq/0"
                )
            self.month = self._io.read_bits_int_le(4)
            if not self.month >= 1:
                raise kaitaistruct.ValidationLessThanError(
                    1, self.month, self._io, "/types/date/seq/1"
                )
            if not self.month <= 12:
                raise kaitaistruct.ValidationGreaterThanError(
                    12, self.month, self._io, "/types/date/seq/1"
                )
        ...
        def padded_day(self):
            ...
            self._m_padded_day = ("0" if self.day <= 9 else "") + str(self.day)
        ...
        def padded_month(self):
            ...
            self._m_padded_month = ("0" if self.month <= 9 else "") + str(self.month)
        ...
        def padded_year(self):
            ...
            self._m_padded_year = (
                "0" + ("0" + ("0" if self.year <= 9 else "") if self.year <= 99 else "")
                if self.year <= 999
                else ""
            ) + str(self.year)
