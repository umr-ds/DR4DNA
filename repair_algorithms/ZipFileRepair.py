...
class ZipFileRepair(FileSpecificRepair):
        ...
        while iters < iterations:
            ...
            try:
                ...
                if err.src_path == "/types/pk_section/seq/0":
                    ...
                    zip_bytes = (
                        zip_bytes[: err.io.pos() - len(err.expected)]
                        + err.expected
                        + zip_bytes[err.io.pos() :]
                    )
                    ...
                    for i, diff in enumerate(
                        [a - b for a, b in zip(expected, struct.pack("<H", err.actual))]
                    ):
                        error_pos[
                            (start * self.semi_automatic_solver.decoder.GEPP.b.shape[0])
                            + err.io.pos()
                            - len(expected)
                            + i
                        ] = (1 if diff != 0 else 0)
                    ...
                    for i, diff in enumerate(
                        [a - b for a, b in zip(expected, struct.pack("<B", err.actual))]
                    ):
                        error_pos[
                            (start * self.semi_automatic_solver.decoder.GEPP.b.shape[0])
                            + err.io.pos()
                ...
                if err.args[0] in [
                    ...
                    "/types/extra/invalid",
                    "/types/len_extra/notmultiple4",
                    "/types/raw_extra/invalid",
                    "/types/raw_body/invalid",
                    "/types/comment_len/invalid",
                    "/types/comment/invalid",
                ]:
    ...
    def repair(self, *args, **kwargs):
        ...
        diff = (
            self.semi_automatic_solver.decoder.GEPP.b[
                start : self.semi_automatic_solver.decoder.number_of_chunks, :
            ].reshape(-1)[:-last_chunk_garbage]
            - self.reconstructed_zip_bytes
        )
        diff_matrix = np.zeros(
            self.semi_automatic_solver.decoder.GEPP.b.shape, dtype=np.uint8
        ).reshape(-1)
        diff_matrix[
            start
            * self.semi_automatic_solver.decoder.GEPP.b.shape[1] : start
            * self.semi_automatic_solver.decoder.GEPP.b.shape[1]
            + len(diff)
        ] = diff
        ...
        for invalid_row in invalid_rows:
            ...
            if len(known_false_columns) > 0:
                repaired_content = bytearray(
                    [
                        ...
                        for i, j in zip(self.gepp.b[invalid_row], diff_matrix[invalid_row])
                    ]
                )
            for column in known_false_columns:
                ...
                tmp_common_packets = self.semi_automatic_solver.decoder.GEPP.get_common_packets(
                    tmp_invalid_rows,
                    tmp_valid_rows,
    ...
    def find_error_region(self, *args, **kwargs):
        ...
        def update_error_pos(
            ...
            error_pos[_start + offset : _end + offset] = [
                new_error_pos[pos] if (i == -1 or overwrite) else i
                for pos, i in enumerate(_parser_error_pos)
            ]
            ...
            return (
                1 if len(np.array(new_error_pos)[np.where(np.array(new_error_pos) > 0)]) > 0 else 0
            )
        ...
        for section in self.zip_structure.sections:
            ...
            if isinstance(section.body, Zip.LocalFile):
                error_counter += update_error_pos(
                    section.body.start - 4, section.body.start, [0] * 4
                )  # PK header will be correct
                ...
                if section.body.header.crc32 == actual_crc32:
                    if (
                        len(data) == section.body.header.len_body_uncompressed
                        or section.body.header.len_body_uncompressed == 0xFFFFFFFF
                    ):
                        # uncompressed size:
                        error_counter += update_error_pos(
                            section.body.header.start + 18,
                            section.body.header.start + 18 + 4,
                            [0] * 4,
                        )
                    else:
                        error_counter += update_error_pos(
                    ...
                    # compression method:
                    error_counter += update_error_pos(
                        section.body.header.start + 4, section.body.header.start + 4 + 2, [0] * 2
                    )
                    # crc32:
                    error_counter += update_error_pos(
                        section.body.header.start + 10, section.body.header.start + 10 + 4, [0] * 4
                    )
                    # body:
                    error_counter += update_error_pos(
                ...
                if (
                    section.body.header.flags.reserved_1 != 0
                    or section.body.header.flags.reserved_2 != 0
                    or section.body.header.flags.reserved_3 != 0
                    or section.body.header.flags.reserved_4 != 0
                ):
                    # we might need to check if offset is correct for all reserved fields
                    error_counter += update_error_pos(
                        section.body.header.start + 2 + 1, section.body.header.start + 2 + 2, [1]
                    )
                ...
                # compressed size:
                if (
                    section.body.header.len_body_compressed == len(section.body.body)
                    or section.body.header.len_body_compressed == 0xFFFFFFFF
                ):
                    error_counter += update_error_pos(
                        section.body.header.start + 14, section.body.header.start + 14 + 4, [0] * 4
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.header.start + 14, section.body.header.start + 14 + 4, [1] * 4
                    )
                # filename:
                if len(section.body.header.file_name) < section.body.header.len_file_name:
                    # non unicode characters were removed from filename...
                    error_counter += update_error_pos(
                        section.body.header.start + 26,
                ...
                next_header_signature = self.get_raw_bytes(
                    section.body.header.start
                    + 24
                    + 2
                    + section.body.header.len_file_name
                    + section.body.header.len_extra
                    + section.body.header.len_body_compressed,
                    2,
                )
                if b"".join(next_header_signature) == b"PK":
                    # length of filename and extra field is correct
                    error_counter += update_error_pos(
                        section.body.header.start + 22,
                        section.body.header.start + 22 + 2 + 2,
                        [0] * 4,
                        overwrite=True,
                    )
                else:
                ...
                if section.body.header.len_extra == 0:
                    error_counter += update_error_pos(
                        section.body.header.start + 24, section.body.header.start + 24 + 2, [0] * 2
                    )
                ...
                for local_file in self.zip_structure.sections:
                    if (
                        isinstance(local_file.body, Zip.LocalFile)
                        and local_file.start == section.body.ofs_local_header
                    ):
                        # in chunk_tag: tag content, crc AND header.compression_method as correct
                ...
                if section.body.disk_of_end_of_central_dir != 0:
                    error_counter += update_error_pos(
                        section.body.start, section.body.start + 2, [1] * 2, [0] * 2
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start, section.body.start + 2, [0] * 2
                    )
                if section.body.disk_of_central_dir != 0:
                    error_counter += update_error_pos(
                        section.body.start + 2, section.body.start + 2 + 2, [1] * 2, [0] * 2
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start + 2, section.body.start + 2 + 2, [0] * 2
                    )
                central_sec_count = sum(
                    [
                        1
                        for i in self.zip_structure.sections
                        if isinstance(i.body, Zip.CentralDirEntry)
                    ]
                )
                if (
                    section.body.num_central_dir_entries_on_disk != central_sec_count
                    and section.body.num_central_dir_entries_on_disk != 0xFFFF
                    and section.body.num_central_dir_entries_on_disk
                    != len(self.zip_structure.sections) - 1
                ):
                    error_counter += update_error_pos(
                        section.body.start + 4,
                        section.body.start + 4 + 2,
                        [1] * 2,
                        struct.pack("<H", central_sec_count),
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start + 4, section.body.start + 4 + 2, [0] * 2
                    )
                if (
                    section.body.num_central_dir_entries_total != central_sec_count
                    and section.body.num_central_dir_entries_total != 0xFFFF
                    and section.body.num_central_dir_entries_total
                    != len(self.zip_structure.sections) - 1
                ):
                    error_counter += update_error_pos(
                        section.body.start + 6,
                        section.body.start + 6 + 2,
                        [1] * 2,
                        struct.pack("<H", central_sec_count),
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start + 6, section.body.start + 6 + 2, [0] * 2
                    )
                central_sec_len = sum(
                    [
                        4 + s.body.end - s.body.start
                        for s in self.zip_structure.sections
                        if isinstance(s.body, Zip.CentralDirEntry)
                    ]
                )
                if section.body.len_central_dir != central_sec_len:
                    # cant be sure that the start and end are correct
                    error_counter += update_error_pos(
                        section.body.start + 8, section.body.start + 8 + 4, [-1] * 4
                    )
                else:
                    error_counter += update_error_pos(
                ...
                if not any(
                    ...
                    error_counter += update_error_pos(
                        section.body.start + 12,
                        section.body.start + 12 + 4,
                        [1] * 4,
                        struct.pack("<I", minium_start_of_a_central_dir),
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start + 12, section.body.start + 12 + 4, [0] * 4
                    )
                if len(section.body.comment) != section.body.len_comment:
                    ...
                    error_counter += update_error_pos(
                        section.body.start + 16,
                        section.body.start + 16 + 2,
                        corrected_bytes=struct.pack("<H", len(section.body.comment)),
                    )
                else:
                    error_counter += update_error_pos(
                        section.body.start + 16, section.body.start + 16 + 2, [0] * 2
                    )
            if error_counter > 4:
        ...
        if start:
            ...
            if (
                self.gepp.b[
                    ...
                    self.semi_automatic_solver.headerChunk.last_chunk_length :,
                ].sum()
                == 0
            ):
                update_error_pos(
                ...
                for central_dir_section in sections:
                    if isinstance(central_dir_section.body, Zip.CentralDirEntry):
                        if central_dir_section.body.ofs_local_header == section.start:
                            ...
                            error_counter += update_error_pos(
                                central_dir_section.start + 42,
                                central_dir_section.start + 42 + 4,
                                [0] * 4,
                            )
                            if (
                                central_dir_section.body.version_needed_to_extract
                                == central_dir_section.body.version_made_by
                            ):
                                error_counter += update_error_pos(
                                    central_dir_section.body.start,
                                    central_dir_section.body.start + 2,
                                    [0] * 2,
                                )
                            # version field(s):
                            if (
                                section.body.header.version
                                == central_dir_section.body.version_needed_to_extract
                            ):
                                error_counter += update_error_pos(
                                    section.body.header.start + 2,
                            ...
                            if (
                                section.body.header.compression_method
                                == central_dir_section.body.compression_method
                            ):
                                error_counter += update_error_pos(
                                    section.body.start + 10, section.body.start + 12, [0] * 2
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 12,
                            ...
                            # modification time:
                            if (
                                section.body.header.file_mod_time.time.second
                                == central_dir_section.body.file_mod_time.time.second
                                and section.body.header.file_mod_time.time.minute
                                == central_dir_section.body.file_mod_time.time.minute
                                and section.body.header.file_mod_time.time.hour
                                == central_dir_section.body.file_mod_time.time.hour
                            ):
                                error_counter += update_error_pos(
                                    section.body.start + 12, section.body.start + 12 + 2, [0] * 2
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 14,
                                    central_dir_section.body.start + 14 + 2,
                                    [0] * 2,
                                )
                            # modification date:
                            if (
                                section.body.header.file_mod_time.date.day
                                == central_dir_section.body.file_mod_time.date.day
                                and section.body.header.file_mod_time.date.month
                                == central_dir_section.body.file_mod_time.date.month
                                and section.body.header.file_mod_time.date.year
                                == central_dir_section.body.file_mod_time.date.year
                            ):
                                error_counter += update_error_pos(
                                    section.body.start + 14, section.body.start + 14 + 2, [0] * 2
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 16,
                            ...
                            if section.body.header.crc32 == central_dir_section.body.crc32:
                                error_counter += update_error_pos(
                                    section.body.start + 16, section.body.start + 16 + 4, [0] * 4
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 18,
                            ...
                            if (
                                section.body.header.len_body_compressed
                                == central_dir_section.body.len_body_compressed
                            ):
                                error_counter += update_error_pos(
                                    section.body.start + 20, section.body.start + 20 + 4, [0] * 4
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 22,
                                    central_dir_section.body.start + 22 + 4,
                                    [0] * 4,
                                )
                            else:
                                if all(
                            ...
                            # uncompressed size:
                            if (
                                section.body.header.len_body_uncompressed
                                == central_dir_section.body.len_body_uncompressed
                            ):
                                error_counter += update_error_pos(
                                    section.body.start + 24, section.body.start + 24 + 4, [0] * 4
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 26,
                                    central_dir_section.body.start + 26 + 4,
                                    [0] * 4,
                                )
                            else:
                                if all(
                            ...
                            if (
                                ...
                                error_counter += update_error_pos(
                                    section.body.start + 30, section.body.start + 30 + 2, [0] * 2
                                )
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 32,
                                ...
                                if all(
                                    ...
                                    if central_dir_section.body.len_file_name == len(
                            ...
                            if section.body.header.file_name == central_dir_section.body.file_name:
                                ...
                                if (
                                    len(central_dir_section.body.file_name)
                                    == central_dir_section.body.len_file_name
                                ):
                                    error_counter += update_error_pos(
                                        central_dir_section.body.start + 46,
                                    ...
                                    error_counter += update_error_pos(
                                        ...
                                        [
                                            0
                                            if (
                                                central_dir_section.body.file_name[i]
                                                == section.body.header.file_name[i]
                                            )
                                            else 1
                                            for i in range(
                                                min(
                                    ...
                                    error_counter += update_error_pos(
                                        ...
                                        + central_dir_section.body.len_file_name,
                                        [
                                            0 if (central_dir_section[i] == section[i]) else 1
                                            for i in range(
                                                min(
                                                    section.body.header.len_file_name,
                                                    central_dir_section.body.len_file_name,
                                    ...
                                    self.reconstructed_zip_bytes[
                                        central_dir_section.body.start
                                        + 46 : central_dir_section.body.start
                                        + 46
                                        + central_dir_section.body.len_file_name
                                    ] = section.body.header._raw_file_name
                            # extra field len:
                            ...
                            next_signature = self.get_raw_bytes(
                                ...
                                + 42
                                + central_dir_section.body.len_file_name
                                + central_dir_section.body.len_extra
                                + central_dir_section.body.len_comment,
                                2,
                            )
                            if b"".join(next_signature) == b"PK":
                                # len file name is correct
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 36,
                                    central_dir_section.body.start + 36 + 2,
                                    [0] * 2,
                                )
                                # len extra field is correct
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 38,
                                    central_dir_section.body.start + 38 + 2,
                                    [0] * 2,
                                )
                                # len file comment is correct
                                error_counter += update_error_pos(
                                    central_dir_section.body.start + 40,
                                    central_dir_section.body.start + 40 + 2,
                                    [0] * 2,
                                )
                            # disk number start:
                            ...
                            if (
                                b"".join(
                                    self.get_raw_bytes(central_dir_section.body.ofs_local_header, 2)
                                )
                                == b"PK"
                            ):
    ...
    def get_ui_elements(self):
        ...
        return {
            ...
            "btn-zipfile-find-rows": {
                "type": "button",
                "text": "Tag (in)correct rows",
                "callback": self.find_correct_rows,
                "updates_b": True,
            },
            "btn-zipfile-find-columns": {
                "type": "button",
                "text": "Tag (in)correct columns",
                "callback": self.find_incorrect_columns,
                "updates_b": True,
            },
            "btn-zipfile-repair": {
                "type": "button",
                "text": "Create corrected version(s)",
    ...
    def find_correct_rows(self, *args, **kwargs):
        ...
        for i in range(min(len(self.chunk_tag), len(res))):
            if res[i]:
                self.chunk_tag[i] = 2
        return {
            "chunk_tag": self.find_incorrect_rows(chunk_tag=self.chunk_tag)["chunk_tag"],
            "updates_b": False,
            "refresh_view": True,
        }
    ...
    def parse_section(self, zip_bytes, start_offset, error_pos, start):
        ...
        while iterations < 100:
            try:
                res = Zip.PkSection(
                    KaitaiStream(io.BytesIO(zip_bytes[start_offset:])), start_offset=start_offset
                )
                ...
                if err.src_path == "/types/pk_section/seq/0":
                    # replace magic bytes with correct ones:
                    zip_bytes = (
                        zip_bytes[: start_offset + err.io.pos() - len(err.expected)]
                        + err.expected
                        + zip_bytes[start_offset + err.io.pos() :]
                    )
                    for i, diff in enumerate([a - b for a, b in zip(err.expected, err.actual)]):
                        error_pos[
                            start_offset
                ...
                if err.args[0] in [
                    ...
                    expected = struct.pack("<H", err.expected)
                    zip_bytes = (
                        zip_bytes[: start_offset + err.error_pos]
                        + expected
                        + zip_bytes[start_offset + err.error_pos :]
                    )
