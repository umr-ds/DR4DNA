...
class LangaugeToolTextRepair(FileSpecificRepair):
    ...
    def detect_language(self, *args, **kwargs):
        ...
        text = self.filter_nonprintable(
            "".join(
                [
                    chr(x)
                    for x in self.gepp.b[start_pos:, :].reshape(-1)[self.gepp.b.shape[1] : 1000]
                ]
            )
        )
    ...
    def find_error_region_by_words(self, language=None, *args, **kwargs):
        ...
        if self.tool is None or self.tool.language.normalized_tag != lang_to_LanguageTool.get(
            language, "en-US"
        ):
            self.tool = language_tool_python.LanguageTool(
                lang_to_LanguageTool.get(language, "en-US")
            )
        ...
        blob = "".join(
            [
                chr(x)
                for x in self.gepp.b[start_pos:, :].reshape(-1)[
                    0 : self.no_inspect_chunks * self.gepp.b.shape[1]
                ]
            ]
        )
        chars_appended = 0
        if self.no_inspect_chunks < self.gepp.b.shape[0]:
            ...
            tmp = "".join(
                [
                    chr(x)
                    for x in self.gepp.b[start_pos:, :].reshape(-1)[
                        (self.no_inspect_chunks * self.gepp.b.shape[1]) : (
                            (self.no_inspect_chunks + 1) * self.gepp.b.shape[1]
                        )
                    ]
                ]
            ).split(" ")[0]
        ...
        for matching_rule in matches:
            ...
            if matching_rule.category == "TYPOS" or matching_rule.ruleIssueType == "misspelling":
                if self.replace_ae_oe_ue:
                    candidates = [
                        x.replace("oe", "ö").replace("ae", "ä").replace("ue", "ü")
                        for x in matching_rule.replacements
                    ]
                    un_escaped = (
                        matching_rule.matchedText.replace("oe", "ö")
                        .replace("ae", "ä")
                        .replace("ue", "ü")
                    )
                else:
                ...
                if len(possible_substitutions) == 0:
                    ...
                    sort_func = functools.partial(
                        lambda x, tkn: levenshtein_distance(x, tkn), tkn=token
                    )
                    correction = sorted(possible_substitutions, key=sort_func, reverse=False)[0]
                    try:
        ...
        for i in range(len(blob)):
            filter_func = (
                lambda x: (x not in string.printable)
                if language == "en-US"
                else (x in nonprintable)
            )
        ...
        if start_pos == 1:
            pos_correct = np.vstack(
                (np.zeros((1, self.gepp.b.shape[1]), dtype=np.float32), pos_correct)
            )
        print(pos_correct)
    ...
    def repair(self, *args, **kwargs):
        ...
        if self.no_columns_to_repair is None or self.no_columns_to_repair == 0:
            ...
            incorrect_columns = sorted(tmp, key=lambda x: x[2], reverse=True)[
                : min(len(tmp), self.no_columns_to_repair)
            ]
        for i in range(len(self.error_matrix)):
            if self.error_matrix[i, incorrect_columns[0][0]] == incorrect_columns[0][1]:
                tmp_b_i = self.gepp.b[i].copy()
                tmp_b_i[incorrect_columns[0][0]] = np.bitwise_xor(
                    tmp_b_i[incorrect_columns[0][0]], int(incorrect_columns[0][1])
                )
                return {
                    "updates_b": True,
                    "repair": {"corrected_row": i, "corrected_value": tmp_b_i},
                    "refresh_view": True,
                    "chunk_tag": self.chunk_tag,
                }
    ...
    def get_ui_elements(self):
        return {
            ...
            "txt-textfile-lt-analyze-row-count": {
                "type": "int",
                "text": "Number of rows to analyze",
                "callback": self.set_no_inspect_chunks,
            },
            "btn-textfile-lt-find-incorrect-rows": {
                "type": "button",
                "text": "Tag incorrect rows",
                "callback": self.find_incorrect_rows,
                "updates_b": True,
            },
            "btn-textfile-lt-find-correct-rows": {
                "type": "button",
                "text": "Tag correct rows",
                "callback": self.find_correct_rows,
                "updates_b": True,
            },
            "btn-textfile-lt-find-columns": {
                "type": "button",
                "text": "Tag (in)correct columns",
                "callback": self.get_incorrect_columns,
                "updates_b": False,
            },
            "txt-textfile-lt-repair-column-count": {
                "type": "int",
                "text": "Columns to repair",
                "callback": self.set_no_columns_to_repair,
            },
            "btn-textfile-lt-repair": {
                "type": "button",
                "text": "Repair",
