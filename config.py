...
try:
    ...
    postgres_ip = (
        os.popen(
            ...
            + docker_name
            + '_no-internet | grep "postgres" -A 4 | grep "IPv4Address"'
        )
        .read()
        .split('": "')[1]
        .split("/")[0]
    ...
    redis_ip = (
        os.popen(
            "docker network inspect "
            + docker_name
            + '_no-internet | grep "redis" -A 4 | grep "IPv4Address"'
        )
        .read()
        .split('": "')[1]
        .split("/")[0]
    ...
    redis_ip = (
        os.environ.get("REDIS_SERVER") or "172.18.0.2"
    )  # docker network inspect dnasim_no-internet | grep "redis" -A 4 | grep "IPv4Address"
...
class Config(object):
    ...
    SECRET_ACCOUNT_DELETION_VALIDATION_KEY = os.environ.get(
