...
from exceptions import DecoderException, DecodeError, FileIOException
...
class DecoderService:
    ...
    def get_possible_invalid_chunks(
        self,
        common_packets: List[bool]
    ) -> List[bool]:
        """
    ...
    def all_solutions_by_reordering(
        self,
        common_packets: List[bool],
        only_possible_invalid: bool = False
    ) -> Dict[int, GEPP]:
        """
        ...
        """
        try:
            return self.solver.all_solutions_by_reordering(
                common_packets,
                only_possible_invalid
            )
        except Exception as e:
    ...
    def get_corrupt_chunks_by_packets(
        self,
        packet_ids: List[int],
        current_chunk_tag: Optional[List[int]] = None,
        tag_num: int = 1
    ) -> List[int]:
        """
        ...
        """
        try:
            return self.solver.get_corrupt_chunks_by_packets(
                packet_ids,
                current_chunk_tag,
                tag_num
            )
