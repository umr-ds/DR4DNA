...
class CallbackResponse:
    ...
    def create_standard_response(
        self,
        recalc_view_result: typing.Optional[tuple] = None
    ) -> tuple:
        """
        ...
        if recalc_view_result:
            return (
                ...
                self.canvas_image_content,
                self.kaitai_view
            )
        return (self.info_str,) + self.create_no_update_tuple(11) + (
            self.canvas_image_content,
            self.kaitai_view
        )
...
class PluginCallbackHandler:
        ...
        **kwargs
        ...
        for plugin in plugin_manager.plugin_instances:
            ...
            for key, value in ui.items():
                if trigger_id == key:
                    return self._process_plugin_callback(
                        value, c_ctx, response, *args, **kwargs
                    )
    ...
    def _process_plugin_response(
        ...
        for key, res_value in res.items():
            ...
            if key == "repair":
                ...
                "updates_canvas", "updates_b", "height", "width", "generate_all",
                "correctness_function", "repair_list"
            ]:
    ...
    def _handle_repair_variations(
        self,
        res_value: typing.Dict,
        response: CallbackResponse
    ) -> None:
        """
...
class ButtonCallbackHandler:
    ...
    def handle_calculate_rank_button(self) -> tuple:
        ...
        if rank_augmented_matrix < solver.decoder.number_of_chunks:
            tmp_str = RANK_STATUS_PARTIAL_RECOVERY.format(
                rank_augmented_matrix,
                solver.decoder.number_of_chunks
            )
        else:
            tmp_str = RANK_STATUS_SOLVABLE
        
        status_msg = (
            RANK_STATUS_DETECTABLE if rank_a != rank_augmented_matrix
    ...
    def handle_save_button(self) -> tuple:
        ...
        try:
            filename = solver.decoder.saveDecodedFile(
    ...
    def handle_packet_tag_buttons(
        self,
        trigger_id: str,
        packet_tag_chunk_input: typing.Union[str, int, None]
    ) -> tuple:
        """
        ...
        self.update_chunk_tag(
            self.solver.get_corrupt_chunks_by_packets(
                [packet_id],
                self.get_chunk_tag(),
                tag_num
            )
    ...
    def handle_repair_reorder_buttons(self, trigger_id: str) -> tuple:
        ...
        return self._process_reorder_solutions(
            only_possible_invalid_packets,
            gepp_backup,
            state,
            fast_most_common_matrix
        )
    ...
    def _process_reorder_solutions(
        ...
        result = self._save_differing_solutions(
            mapping,
            differing_gepp_ids,
            working_dir
        )
        ...
        self._update_chunk_tags_from_solutions(
            ...
            differing_gepp_ids,
            gepp_backup,
            fast_most_common_matrix_func
        )
        
        response.info_str = result
        return response.create_standard_response(self.recalculate_view())
    
    def _find_differing_solutions(
        self,
        mapping: typing.Dict
    ) -> set:
        """
    ...
    def _save_single_solution(
        ...
        is_correct = (
            solver.headerChunk is not None and
            solver.headerChunk.checksum_len_format is not None and
            solver.is_checksum_correct()
        )
    ...
    def _update_chunk_tags_from_solutions(
        ...
        chunk_tag = self.get_chunk_tag()
        valid_rows = [
            i for i, v in enumerate(np.all(has_single_val, axis=1))
    ...
    def handle_forceload_plugin_button(self, c_ctx: typing.Any) -> tuple:
        ...
        for plugin in plugin_manager.plugin_instances:
            if plugin.__class__.__name__ == c_ctx.triggered_id["index"]:
                ...
                canvas_style = (
                    {"display": "block"} if state.show_canvas else {"display": "none"}
                )
...
class RepairCallbackHandler:
    ...
    def __init__(
        self,
        repair_callback_func: typing.Callable,
        repair_chunks_func: typing.Callable
    ):
